...
def collectMeshes(context, bones, scale, matfun, useskel = True, usecol = False, usemods = False, filetype = 'IQM', flipyz = False):
    vertwarn = []
...
def exportIQM(context, filename, usemesh = True, usemods = False, useskel = True, usebbox = True, usecol = False, scale = 1.0, animspecs = None, matfun = (lambda prefix, image: image), derigify = False, boneorder = None, flipyz = False):
    ...
    if usemesh:
        meshes = collectMeshes(context, bones, scale, matfun, useskel, usecol, usemods, filetype, flipyz)
    else:
        meshes = []
